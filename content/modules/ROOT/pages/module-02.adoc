...
* Explain how physically-bound images embed container images for disconnected operation
* Build a base RHEL image with MicroShift using podman
* Build a self-contained appliance image with embedded applications
* Generate a bootable ISO from a container image using bootc-image-builder
...
[source,sh,role=execute]
----
ls -lh /var/tmp/bootc-images/bootiso/*.iso
----
...
* ISO file `microshift-4.19-bootc-embeeded-v1.iso` should exist in `/var/tmp/bootc-images/bootiso/` (or `_build/` directory)
* ISO file size should be several GB (contains embedded container images)

If any of these are missing, review the build logs (`build-base.log` and `build-v1.log`) for errors.
====
