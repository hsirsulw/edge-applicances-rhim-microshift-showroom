...
* Known-good versions for years
* Fewer surprise updates
* Reduced validation and testing load

For industrial systems, stability beats speed — making EUS the natural choice.

=== Why Embed Repo Metadata (`redhat.repo`)

During image builds, tools like `dnf` must resolve packages.

Embedding repo metadata ensures:

* Builds complete without external repo configuration
* Reproducible builds across CI/CD pipelines
* No dependency on host-side repo files later

This is required for repeatable container-based OS builds.
...
* Bundles multiple images
* Filesystem-friendly, no registries needed
* Preserves digests (critical for SHA-pinned references)
