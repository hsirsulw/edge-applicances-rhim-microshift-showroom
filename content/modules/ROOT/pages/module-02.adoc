...
| **Appliance Image** | `microshift-bootc-embedded:4.19` exists in podman.
| **Bootable ISO** | `microshift-bootc-embedded-4.19.iso` is created and >5GB.
|===

[source,sh,role=execute]
----
sudo podman images | grep microshift-bootc
ls -lh microshift-bootc-embedded-4.19.iso
----
