...
RHEL Image Mode addresses these challenges by treating the *entire* OS as an immutable, versioned image. Instead of updating individual packages, you deploy a new OS image exactly like updating a container. This approach provides:
...
* **Disconnected Operation:** Self-contained images can operate in air-gapped or intermittently connected environments
...
* `Containerfile.base`, `Containerfile.v1`, `Containerfile.v2`, `Containerfile.v3` - Containerfile definitions for building different versions
...
* `create-ks.sh`, `create-network.sh`, `create-vm.sh` - Scripts for VM creation
