...
ManufacturingCo—and most organizations deploying systems outside the data center—face the same reality:
...
Inconsistent environments amplify the challenges already introduced in the lab overview:
...
* *Manufacturing:* ManufacturingCo uses edge appliances to deploy quality monitoring systems to 50 factory locations, but faces 6-8 week deployment cycles and 15% update failure rates requiring on-site IT intervention.  
* *Telecom:* TelecomProvider deploys network edge appliances for 5G base stations, enabling rapid deployment and updates across hundreds of cell sites with minimal on-site IT support.
* *Energy:* EnergyCorp uses industrial IoT gateways for oil & gas monitoring, ensuring reliable operation in remote locations with intermittent connectivity and harsh environmental conditions.
* *Healthcare:* HealthSystem deploys medical device edge appliances for patient monitoring, maintaining HIPAA compliance and ensuring consistent security patches across all hospital locations.
====
...
RHEL Image Mode **treats the OS like a container image** using bootc.
Instead of modifying a live filesystem, appliances update by replacing the system image entirely.

This provides:

* **Consistency:** Every appliance runs the same bootc image  

image::manage-fleet.jpg[manage-fleet,100%,100%]

* **Reliability:** Updates are transactional with instant rollback  

image::updates.jpg[updates-workflow,100%,100%]

* **Simplicity:** Same tooling used for application development (Containerfiles, registries, CI/CD)

* **Disconnected Operation:** Images embed OS + MicroShift + apps → no runtime pulls  

image::disconnected.jpg[disconnected-workflow,100%,100%]

* **Resource Optimization:** Minimal overhead, ideal for small footprint devices
...
* **Container Build Host** –  
  ...
  You will use podman, buildah, and skopeo to:
