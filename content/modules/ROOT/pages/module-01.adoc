...
| *Describe*
| Manual requirements or complex, scattered install scripts.
| A single, versioned `Containerfile` defines the entire OS + App.

| *Build*
| Building a fresh OS often requires dedicated, complex build servers.
| Standard container tools (Podman/Docker) build the OS like an app.

| *Generate*
| Manual ISO creation or conversion tools; hard to automate.
| `bootc-image-builder` outputs ISO, AMI, or QCOW2 in one step.

| *Deploy*
| Manual installation, PXE booting, or separate config management.
| Deploy the same image to VM, Bare Metal, or Edge devices.

| *Update*
| Incremental (drift-prone) package updates or risky manual patching.
| Atomic "A/B" updates: pull a new image and reboot to swap.
|===
