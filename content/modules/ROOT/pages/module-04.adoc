...
Now let's create an updated version of our appliance: **4.20**.  
...
. Confirm 4.20 is now the active deployment:
+
[source,sh,role=execute]
----
sudo bootc status
----
+
The active deployment should show `dir:/var/tmp/microshift-bootc-4.20/` as the booted image.
...
* `bootc status` should show 4.20 (dir:/var/tmp/microshift-bootc-4.20/) as the active deployment
...
**Delta updates** solve this by transferring only the difference between versions (4.19 → 4.20).
